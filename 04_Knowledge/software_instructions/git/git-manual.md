...
- [1. 核心概念（必须懂）](#1-核心概念必须懂)
  - [1.1 三个区](#11-三个区)
  - [1.2 文件的三种常见状态](#12-文件的三种常见状态)
- [2. 日常工作流（推荐你每天就用这套）](#2-日常工作流推荐你每天就用这套)
  - [2.1 开始一天工作（先同步）](#21-开始一天工作先同步)
  - [2.2 开新分支（真实例子：修复“项目报告下载”）](#22-开新分支真实例子修复项目报告下载)
- [3. PR / MR 是什么？](#3-pr--mr-是什么)
- [4. 常用查看命令（排错必用）](#4-常用查看命令排错必用)
  - [4.1 看当前状态](#41-看当前状态)
  - [4.2 看改了什么](#42-看改了什么)
  - [4.3 看提交历史](#43-看提交历史)
  - [4.4 查看某个文件的修改历史](#44-查看某个文件的修改历史)
- [5. 撤销 / 回退（最常用也最容易搞错）](#5-撤销--回退最常用也最容易搞错)
  - [5.1 丢弃“所有未提交修改”（含已 add）](#51-丢弃所有未提交修改含已-add)
  - [5.2 只丢弃工作区修改（不动暂存区）](#52-只丢弃工作区修改不动暂存区)
  - [5.3 取消暂存（把 add 撤回到工作区）](#53-取消暂存把-add-撤回到工作区)
  - [5.4 删除未跟踪文件（Untracked files）](#54-删除未跟踪文件untracked-files)
  - [5.5 先存起来（不想丢改动：WIP）](#55-先存起来不想丢改动wip)
  - [5.6 误操作补救（找回丢失提交/分支）](#56-误操作补救找回丢失提交分支)
- [6. 分支管理（协作核心）](#6-分支管理协作核心)
  - [6.1 查看分支](#61-查看分支)
  - [6.2 删除分支](#62-删除分支)
  - [6.3 清理已删除的远端分支引用](#63-清理已删除的远端分支引用)
- [7. 同步远端（pull / fetch / rebase 的正确姿势）](#7-同步远端pull--fetch--rebase-的正确姿势)
  - [7.1 pull 的本质](#71-pull-的本质)
  - [7.2 更稳的方式（推荐）](#72-更稳的方式推荐)
  - [7.3 rebase（让提交历史更干净：个人分支推荐）](#73-rebase让提交历史更干净个人分支推荐)
- [8. 冲突处理（merge/rebase 常见）](#8-冲突处理mergerebase-常见)
  - [8.1 冲突出现后](#81-冲突出现后)
  - [8.2 标准流程](#82-标准流程)
  - [8.3 放弃本次 merge/rebase](#83-放弃本次-mergerebase)
- [9. 远端与认证（GitHub 常见坑）](#9-远端与认证github-常见坑)
  - [9.1 查看 remote](#91-查看-remote)
  - [9.2 认证失败：账号密码不行了怎么办？](#92-认证失败账号密码不行了怎么办)
- [10. .gitignore（工程必须配置）](#10-gitignore工程必须配置)
- [11. Tag / Release（交付与版本追溯）](#11-tag--release交付与版本追溯)
  - [11.1 打 tag（语义化版本）](#111-打-tag语义化版本)
  - [11.2 查看 tag](#112-查看-tag)
- [12. 回滚策略（生产更安全）](#12-回滚策略生产更安全)
  - [12.1 推荐：revert（公共分支首选）](#121-推荐revert公共分支首选)
  - [12.2 reset（改历史：慎用公共分支）](#122-reset改历史慎用公共分支)
- [13. 提交规范（建议统一：Conventional Commits）](#13-提交规范建议统一conventional-commits)
- [14. 团队协作分支模型（轻量 GitFlow，适合商业交付）](#14-团队协作分支模型轻量-gitflow适合商业交付)
  - [14.1 分支角色](#141-分支角色)
  - [14.2 一句话规则](#142-一句话规则)
- [15. 最小命令集合（记住这 10 条就够用）](#15-最小命令集合记住这-10-条就够用)
